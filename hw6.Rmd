...
cities_plot =
  ...
  geom_point() +
  labs(
    title = "Estimated OR and CIs for All Cities",
    x = "City",
    y = "OR Estimates"
  )  +
...
set.seed(12345)
```
```{r, uploading and cleaning, warning = FALSE, echo = FALSE}
...
```
```{r, maternal factors bmi and wt, warning = FALSE, echo = FALSE, warning = FALSE, error = FALSE}
mod_maternalfactors = 
  birthweight_data %>%
...
```
The modeling process here chose maternal bmi and pre-pregnancy weight in pounds. It is theorized that maternal physical condition can influence the neonate's birthweight. 
```{r length and gestation age, warning = FALSE, echo = FALSE}
...
mod_birthleng_gestweek %>%
  head() %>%
  modelr::add_residuals(lm_birthleng_gestweek) %>%
  modelr::add_predictions(lm_birthleng_gestweek)
```
```{r head circum length and sex, warning = FALSE, echo = FALSE}
mod_headlengthsex = 
  birthweight_data %>%
...
mod_headlengthsex %>%
  head() %>%
  modelr::add_residuals(lm_headlengthsex) %>%
  modelr::add_predictions(lm_headlengthsex)
```
```{r crossvalidation of models, warning = FALSE, echo = FALSE}
